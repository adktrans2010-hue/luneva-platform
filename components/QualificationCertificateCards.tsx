      ...
      <div className="mt-8 grid gap-4">
        {cards.map((card) => {
          ...
          return card.externalUrl ? (
            <a key={card.id} href={card.externalUrl} target="_blank" rel="noopener noreferrer" data-certificate-card-id={card.id} aria-label={`${card.title}. ${card.actionLabel}`} className={className} onClick={() => trackGoal("certificate_card_click")}>
              {content}
            </a>
          ) : (
